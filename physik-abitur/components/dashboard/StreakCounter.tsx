...
export function StreakCounter() {
  const streak = useProgressStore((s) => s.progress?.streak) ?? { current: 0, lastActive: new Date(0), longest: 0 };
  ...
  return (
    <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-orange-500/20 border border-orange-500/40">
      ...
      <span className="text-sm font-medium">
        <span className="text-orange-400">{streak?.current ?? 0}</span>
        <span className="text-text-dim ml-1">
          {(streak?.current ?? 0) === 1 ? 'Tag' : 'Tage'} in Folge
        </span>
      </span>
    </div>
  );
